<template>
  <div class="py-5 add-request" style="font-family: 'cygre', serif; font-size: 17px">
        ...
        <div class="p-6 bg-white card-body">
          ...
          <div class="mb-4">
            <div v-for="(item, index) in requestData.minute_point" :key="index" class="flex gap-4 mb-4">
              <div class="w-full">
                <label class=" form-label">Minute Point/Current Update</label>
          ...
          <div class="mt-4">
            <label class="block text-sm  text-gray-700">Plans/Progress</label>
          ...
          <!-- Select Engineer -->
          <div class="mt-4">
            <label class="form-label">Select Department Engineer</label>
          ...
          <div class="flex gap-4 mt-4">
            ...
            <router-link to="/meetingmenutes" class="flex items-center gap-2 px-4 py-2 text-gray-700 bg-gray-200 rounded hover:bg-gray-300">
...
async function applyMeetingMinutes() {
  ...
  try {
    ...
    resetForm();
    router.push('/create/meetingmenutes');
  } catch (error) {
    handleError(error);
  } finally {
    isLoading.value = false;
