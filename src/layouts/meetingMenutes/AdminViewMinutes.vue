<template>
  <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Meeting Minutes Report</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          View and export meeting minutes by selected date
        </p>
      </div>
    </div>
    ...
    <div v-if="isLoading" class="bg-white dark:bg-gray-900 shadow-sm rounded-xl p-12 text-center">
      ...
      <p class="mt-4 text-gray-600 dark:text-gray-400">Loading meeting minutes...</p>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Department Engineer</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Prepared By</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Project</th>
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Minute Points</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-gray-900">
            <tr
              v-for="(minute, index) in paginatedData"
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
                {{ minute.logged_user?.name || '—' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                {{ minute.project?.project_name || '—' }}
              </td>
              <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                {{ minute.minute_point || '—' }}
              </td>
              <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                {{ minute.if_more_detail || '—' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300">
            ...
            <tr v-if="paginatedData.length === 0">
              <td colspan="7" class="px-6 py-16 text-center text-gray-500 dark:text-gray-400">
                No meeting minutes found for the selected date
              </td>
            </tr>
          </tbody>
        </table>
  ...
  try {
    ...
    if (response.data.status === 'success') {
      ...
      if (reportData.value.length === 0) {
        toast.info('No meeting minutes found for the selected date')
      }
    } else {
...
function exportToExcel() {
  ...
  const worksheet = XLSX.utils.json_to_sheet(
    reportData.value.map((minute, index) => ({
      No: index + 1,
      'Department Engineer': minute.user?.name || 'N/A',
      'Prepared By': minute.logged_user?.name || 'N/A',
      Project: minute.project?.project_name || 'N/A',
      'Minute Points': minute.minute_point || 'N/A',
      Details: minute.if_more_detail || 'N/A',
      Date: minute.created_at ? formatDate(minute.created_at) : 'N/A'
    }))
  )
...
function exportToPDF() {
  ...
  const doc = new jsPDF()
  doc.setFontSize(16)
  doc.text('Meeting Minutes Report', 14, 20)

  autoTable(doc, {
    head: [['No', 'Department Engineer', 'Prepared By', 'Project', 'Minute Points', 'Details', 'Date']],
    body: reportData.value.map((minute, index) => [
      index + 1,
      ...
      minute.created_at ? formatDate(minute.created_at) : 'N/A'
    ]),
    startY: 30,
