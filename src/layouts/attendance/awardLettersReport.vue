<template>
  <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Award Letter Reports</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Search and export award letter reports by date range
        </p>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 shadow-sm rounded-xl border border-gray-200 dark:border-gray-800 p-5">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 items-end">
        ...
        <div class="flex flex-col sm:flex-row gap-3 sm:items-end">
          ...
          <button
            @click="exportToExcel"
            :disabled="isLoading || !awardLetters.length"
    ...
    <div v-else-if="isLoading" class="bg-white dark:bg-gray-900 shadow-sm rounded-xl p-12 text-center">
      ...
      <p class="mt-4 text-gray-600 dark:text-gray-400">Loading award letter reports...</p>
    </div>

    <!-- Table (when data exists) -->
    <div v-else-if="awardLetters.length > 0" class="bg-white dark:bg-gray-900 shadow-sm rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Award Letter File</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-gray-900">
            <tr
              v-for="(award, index) in paginatedData"
              :key="award.award_id"
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                <button
                  v-if="award.award_letter_file"
                  @click="viewAwardFile(award.award_letter_file)"
    ...
    <div v-else class="bg-white dark:bg-gray-900 shadow-sm rounded-xl p-12 text-center">
      ...
      <p class="mt-1 text-gray-500 dark:text-gray-400">
        No award letters match the selected date range
      </p>
    </div>

    <!-- Pagination -->
    <div v-if="awardLetters.length > 0" class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
...
// View / Download award letter file (opens in new tab)
function viewAwardFile(url) {
  if (!url) {
...
function exportToExcel() {
  ...
  const worksheet = XLSX.utils.json_to_sheet(
    paginatedData.value.map((entry, index) => ({
      ...
      'Award Letter File': entry.award_letter_file || 'N/A',
...
function exportToPDF() {
  ...
  const doc = new jsPDF()
  doc.setFontSize(18)
  doc.text('Award Letter Reports', 14, 22)
