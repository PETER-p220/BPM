<template>
  <div class="min-h-screen page-bg" style="font-family: 'DM Sans', sans-serif;">
    ...
    <!-- Sticky Header -->
    <header class="sticky top-0 z-30 header-glass border-b border-gray-100/80 backdrop-blur-xl">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">
              <span class="font-semibold text-indigo-600">{{ reportData.length }}</span> record{{ reportData.length !== 1 ? 's' : '' }}
        ...
        <router-link
          to="/accountant-create-attendance"
    ...
    <main class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <transition name="fade">
        <div v-if="reportData.length > 0" class="summary-row fade-up">
          ...
          <div class="summary-card s-present">
            <div class="s-icon s-icon-present">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div>
              <div class="s-num text-green-600">{{ presentCount }}</div>
              <div class="s-label">Present</div>
            </div>
          </div>

          <div class="summary-card s-absent">
            <div class="s-icon s-icon-absent">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div>
              <div class="s-num text-red-600">{{ absentCount }}</div>
              <div class="s-label">Absent</div>
            </div>
          </div>
        </div>
      </transition>
                ...
                >
                  ...
                  <td>
                    <div class="engineer-cell">
                      <div class="eng-avatar">{{ att.user?.name?.[0] || '?' }}</div>
                      <span class="eng-name">{{ att.user?.name || '—' }}</span>
                    </div>
                  </td>
                  <td>
                    <span
                      class="status-badge"
                      :class="att.is_present === 'present' ? 'badge-present' : 'badge-absent'"
                    >
                      <span class="badge-dot"></span>
                      {{ att.is_present === 'present' ? 'Present' : 'Absent' }}
                    </span>
                  </td>
                  <td class="td-reason">
                    {{ att.if_not_present_and_have_reason || '—' }}
                  </td>
                  <td class="td-date">
      ...
      <transition name="fade">
        ...
        >
          ...
          <h3 class="text-xl font-bold text-gray-800 mt-6">No Records Found</h3>
          <p class="text-gray-500 mt-2 max-w-md mx-auto">
            No attendance data available for <strong>{{ formattedDate }}</strong>.
          </p>
        </div>
      </transition>
      ...
      <transition name="fade">
        <div v-if="loading" class="loading-overlay">
          ...
          <p class="mt-4 text-gray-600 font-medium">Loading attendance report...</p>
        </div>
      </transition>
    </main>
  </div>
...
const presentCount = computed(() => reportData.value.filter(r => r.is_present === 'present').length)
const absentCount  = computed(() => reportData.value.filter(r => r.is_present !== 'present').length)
...
async function fetchReport() {
  ...
  try {
    ...
    toast.error('Could not fetch attendance report')
    console.error(err)
  } finally {
    loading.value = false
  }
...
function exportToExcel() {
  ...
  const data = reportData.value.map((r, i) => ({
    ...
    Status: r.is_present === 'present' ? 'Present' : 'Absent',
    Reason: r.if_not_present_and_have_reason || '—',
    'Recorded At': formatDate(r.created_at)
  }))

  const ws = XLSX.utils.json_to_sheet(data)
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, 'Attendance')
  XLSX.writeFile(wb, `Attendance_${selectedDate.value || 'Report'}.xlsx`)
...
function exportToPDF() {
  ...
  doc.text(`Attendance Report – ${formattedDate.value}`, 14, 22)
  ...
  const tableColumn = ['#', 'Engineer', 'Status', 'Reason', 'Recorded At']
  const tableRows = reportData.value.map((r, i) => [
    i + 1,
    r.user?.name || '—',
    r.is_present === 'present' ? 'Present' : 'Absent',
    r.if_not_present_and_have_reason || '—',
  ...
  doc.save(`Attendance_Report_${selectedDate.value || new Date().toISOString().slice(0,10)}.pdf`)
...
.s-icon-total   { background: #eef2ff; color: #4f46e5; }
.s-icon-present { background: #ecfdf5; color: #10b981; }
.s-icon-absent  { background: #fef2f2; color: #ef4444; }
