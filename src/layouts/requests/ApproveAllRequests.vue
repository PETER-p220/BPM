<template>
  ...
  {{ request.is_approved }}
            ...
            <td class="table-data">
      <button @click="approveRequest(request.request_id)" class="text-white px-4 py-2 rounded-full hover:bg-blue-600" style="background-color: #0d4063;">
        Approve
      </button>
    </td>
          </tr>
...
// Function to navigate to ApproveRequest page with request_id
function approveRequest(request_id) {
  router.push({ name: 'ApproveRequest', params: { request_id: request_id } });
}
