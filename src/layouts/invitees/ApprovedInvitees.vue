<template>
        ...
        <table class="w-full divide-y divide-gray-200 rounded-table dark:divide-gray-700" id="data-table" style="box-shadow: rgba(149, 157, 165, 0.2) 0px 8px 24px;">
          ...
          <tbody class="bg-white divide-y divide-gray-200 dark:bg-dark-header dark:divide-gray-700">
            <tr v-for="(invitee, index) in filteredInvitees" :key="invitee.invitee_id">
              ...
              <td class="table-data"> 
                <p class="text-center btn" style="background-color:#1b4f72;color:white">{{ invitee.is_confirmed }}</p>
              </td>
  ...
  async function fetchInvitees() {
    try {
      const response = await axios.get('api/my-invitationsv1');
  ...
  const filteredInvitees = computed(() => {
    return invitees.value.filter(invitee =>
      ...
      invitee.event.event_type.toLowerCase().includes(filter.value.toLowerCase()) ||
      invitee.is_confirmed.toLowerCase().includes(filter.value.toLowerCase()) ||  // Add other fields to filter as needed
      invitee.event_time.toLowerCase().includes(filter.value.toLowerCase()) ||
      invitee.venue.toLowerCase().includes(filter.value.toLowerCase()) ||
      invitee.physical_address.toLowerCase().includes(filter.value.toLowerCase())
    );
  });
