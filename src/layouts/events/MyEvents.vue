<template>
        ...
        <table class="w-full divide-y divide-gray-200 rounded-table dark:divide-gray-700" id="data-table" style="box-shadow: rgba(149, 157, 165, 0.2) 0px 8px 24px;">
          ...
          <tbody class="bg-white divide-y divide-gray-200 dark:bg-dark-header dark:divide-gray-700">
            <tr v-for="(event, index) in filteredEvents" :key="event.event_id">
              ...
              <td class="table-data">
                <i @click="editEvent(event.event_id)" class="fas fa-edit" style="color:#21618c;font-weight:bold;font-size:17px"></i>
                <i @click="deleteEvent(event.event_id)" class="fas fa-trash" style="color: #b53b0a;font-weight:bold;font-size:17px"></i>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  function editEvent(eventId) {
    router.push({ name: 'EditEvent', params: { event_id: eventId } });
  }
  
  // Function to delete an event with toast confirmation
