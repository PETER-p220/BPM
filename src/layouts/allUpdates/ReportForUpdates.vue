<template>
    <div class="p-4 space-y-4" style="font-family: 'cygre', sans-serif; font-size: 17px; margin-top: 60px;">
      <PageHeader title="Updates Reports" subtitle="Please search report by entering date range" class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <table class="min-w-full divide-y py-5 divide-gray-200 rounded-lg" id="receiptTable">
          <thead class="bg-teal-10">
            <tr>
              ...
              <th class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-200">Updates</th>
          ...
          <tbody>
            <tr v-for="(report, index) in paginatedReports" :key="report.chat_id">
              ...
              <td class="px-4 py-2">{{ formatDate(report.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  
      <div v-if="!filteredReports.length" class="text-center text-gray-500">
        No records found.
      </div>
      ...
      <div class="flex justify-center mt-4">
        ...
        <button @click="changePage(currentPage + 1)" :disabled="currentPage * itemsPerPage >= filteredReports.length" class="px-4 py-2 bg-gray-300 rounded">
          Next
        </button>
      </div>
...
function exportToPDF() {
    const doc = new jsPDF();
    const tableColumn = ["No", "Engineer", "Upates", "Description", "Download File", "Submitted At"];
    const tableRows = [];
    ...
    filteredReports.value.forEach((report, index) => {
        const reportData = [
            ...
            report.title,
            report.description,
            report.update_file ? report.update_file : 'No File',
    ...
    doc.text("Chat Reports", 14, 15);
    doc.save('chat_reports.pdf');
}
  
function exportToExcel() {
    const worksheet = XLSX.utils.json_to_sheet(filteredReports.value.map((report, index) => ({
        No: (currentPage.value - 1) * itemsPerPage + index + 1,
        User: report.user.name,
        Title: report.title,
        Description: report.description,
        "Download File": report.update_file ? report.update_file : 'No File',
        "Created At": formatDate(report.created_at),
    })));
  
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Chat Reports");
    XLSX.writeFile(workbook, "chat_reports.xlsx");
}
</script>
