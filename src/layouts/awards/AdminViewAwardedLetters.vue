<template>
  <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Award Letters</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          View and download issued award letters for tenders
        </p>
      </div>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-gray-900">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button
                  v-if="item.awardletter_file"
                  @click="downloadAwardLetterFile(item.awardletter_file)"
            ...
            <tr v-if="paginatedData.length === 0">
              <td colspan="4" class="px-6 py-16 text-center text-gray-500 dark:text-gray-400">
                <div class="flex flex-col items-center">
                  ...
                  <p class="text-lg font-medium">No award letters found</p>
...
const filteredData = computed(() => {
  if (!filter.value.trim()) return awardLetters.value;

  const search = filter.value.toLowerCase();
  return awardLetters.value.filter(item =>
...
async function downloadAwardLetterFile(url) {
  if (!url) {
  ...
  try {
    ...
    link.download = url.split('/').pop() || 'award_letter.pdf';
...
function exportToExcel() {
  ...
  const data = filteredData.value.map((item, index) => ({
    ...
    AwardLetterFile: item.awardletter_file ? 'Yes' : 'No',
    CreatedAt: item.created_at ? new Date(item.created_at).toLocaleString() : 'N/A'
  }));

  const worksheet = XLSX.utils.json_to_sheet(data);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Award Letters');
  XLSX.writeFile(workbook, 'Award_Letters.xlsx');
}
