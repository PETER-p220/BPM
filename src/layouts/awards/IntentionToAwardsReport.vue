<template>
    <div class="p-4 space-y-4" style="font-family: 'cygre', sans-serif; font-size: 17px; margin-top: 60px;">
      <PageHeader title="Intention Reports" subtitle="Please search report by entering date range" class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <div v-else-if="isSearched && intentions.length === 0" class="text-center text-gray-500">
        ...
        <table class="min-w-full divide-y py-5 divide-gray-200 rounded-lg" id="data-table">
          ...
          <tbody>
            <tr v-for="(intention, index) in paginatedData" :key="intention.intention_id">
              ...
              <td class="px-4 py-2">{{ intention.tender.title }}</td>
              <td class="px-4 py-2">{{ intention.user.name }}</td>
              ...
              <td class="px-4 py-2">{{ formatDate(intention.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  
      <div v-if="isSearched && intentions.length > 0" class="flex justify-center mt-4">
        <button
  ...
  async function searchData() {
    ...
    try {
      ...
      const response = await axios.get('api/intention-reports', { params });
  ...
  function exportToExcel() {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Intention Reports');
    XLSX.writeFile(workbook, 'Intention_Reports.xlsx');
  }
  ...
  function exportToPDF() {
    ...
    const doc = new jsPDF();
    const title = 'Intention Reports Data';
    const headers = [['No', 'Tender Title', 'Created By', 'Created At']]; // Excluded Intention File
