<template>
    <div class="p-4 space-y-4" style="font-family: 'cygre', sans-serif; font-size: 17px; margin-top: 60px;">
      <PageHeader title="Security Declaration Reports" subtitle="Please search report by entering date range" class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <div v-else-if="isSearched && securityDeclarations.length === 0" class="text-center text-gray-500">
        ...
        <table class="min-w-full divide-y py-5 divide-gray-200 rounded-lg" id="data-table">
          <thead class="bg-teal-10">
            <tr>
              ...
              <th class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-200">Security Declaration File</th>
              <th class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-200">Created At</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(declaration, index) in paginatedData" :key="declaration.declaration_id">
              <td class="px-4 py-2">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
              <td class="px-4 py-2">{{ declaration.tender.title }}</td>
              <td class="px-4 py-2">{{ declaration.user.name }}</td>
              <td class="px-4 py-2 cursor-pointer">
                <button
                  class="btn-block btn"
                  style="width:140px; background-color:#27ae60; color:white; border-radius:17px"
                  @click="downloadDeclarationFile(declaration.security_declaration_file)"
                >
                  Download PDF
                </button>
              </td>
              <td class="px-4 py-2">{{ formatDate(declaration.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  
      <div v-if="isSearched && securityDeclarations.length > 0" class="flex justify-center mt-4">
        <button
  ...
  function exportToExcel() {
    ...
    const worksheet = XLSX.utils.json_to_sheet(
      paginatedData.value.map((entry, index) => ({
        ...
        SecurityDeclarationFile: entry.security_declaration_file || 'N/A',
  ...
  function exportToPDF() {
    ...
    const doc = new jsPDF();
    const title = 'Security Declaration Reports Data';
    const headers = [['No', 'Tender Title', 'Created By', 'Created At']]; // Excluded Security Declaration File
