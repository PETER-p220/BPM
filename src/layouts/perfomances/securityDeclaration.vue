<template>
  <div class="p-4 space-y-4" style="font-family: 'cygre', sans-serif; font-size: 17px">
    <PageHeader subtitle="Security Declarations">
      ...
      <table class="w-full divide-y divide-gray-200 rounded-table dark:divide-gray-700" id="data-table"
        ...
        <thead class="bg-gray-50 dark:bg-neutral-700"
          ...
          <tr>
            ...
            <th class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-200">Declaration File</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200 dark:bg-dark-header dark:divide-gray-700">
          <tr v-for="(securityDeclaration, index) in paginatedData" :key="securityDeclaration.declaration_id">
            <td class="table-data">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
            <td class="table-data">{{ securityDeclaration.tender.title }}</td>
            <td class="table-data">
            ...
            <td class="table-data">{{ securityDeclaration.receiver_email || 'N/A' }}</td>
            <td class="table-data">{{ securityDeclaration.created_at }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
function exportToExcel() {
  const worksheet = XLSX.utils.json_to_sheet(
    paginatedData.value.map((entry, index) => ({
      ...
      DeclarationFile: entry.declaration_file || 'N/A',
