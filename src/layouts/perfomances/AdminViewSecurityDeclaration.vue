<template>
  <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-screen">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Security Declarations</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          List of all submitted security declarations for tenders
        </p>
      </div>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-gray-900">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <button
                  v-if="item.declaration_file"
                  @click="downloadDeclarationFile(item.declaration_file)"
            ...
            <tr v-if="paginatedData.length === 0">
              <td colspan="4" class="px-6 py-16 text-center text-gray-500 dark:text-gray-400">
                <div class="flex flex-col items-center">
                  ...
                  <p class="text-lg font-medium">No security declarations found</p>
...
async function fetchData() {
  try {
    ...
    toast.error(error.response?.data?.message || 'Failed to load security declarations');
  }
}

// Computed properties
const filteredData = computed(() => {
  if (!filter.value.trim()) return securityDeclarations.value;

  const search = filter.value.toLowerCase();
  return securityDeclarations.value.filter(item =>
...
async function downloadDeclarationFile(url) {
  if (!url) {
  ...
  try {
    ...
    link.download = url.split('/').pop() || 'security_declaration.pdf';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  } catch (err) {
    toast.error('Failed to download security declaration');
  }
}
...
function exportToExcel() {
  ...
  const data = filteredData.value.map((item, index) => ({
    ...
    DeclarationFile: item.declaration_file ? 'Yes' : 'No',
...
function exportToPDF() {
  ...
  const doc = new jsPDF();
  doc.setFontSize(16);
  doc.text('Security Declarations Report', 14, 20);

  const tableData = filteredData.value.map((item, index) => [
    index + 1,
    item.tender?.title || '—',
    item.declaration_file ? 'Yes' : 'No',
