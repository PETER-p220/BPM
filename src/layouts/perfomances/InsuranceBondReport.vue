<template>
    <div class="p-4 space-y-4" style="font-family: 'cygre', sans-serif; font-size: 17px; margin-top: 60px;">
      <PageHeader title="Insurance Bond Reports" subtitle="Please search report by entering date range" class="px-6 py-3 text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <div v-else-if="isSearched && insuranceBonds.length === 0" class="text-center text-gray-500">
      ...
      <div v-else class="overflow-x-auto" style="margin-top: 30px;">
        <table class="min-w-full divide-y py-5 divide-gray-200 rounded-lg" id="data-table">
          ...
          <tbody>
            <tr v-for="(bond, index) in paginatedData" :key="bond.insurance_id">
              <td class="px-4 py-2">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
              <td class="px-4 py-2">{{ bond.tender.title }}</td>
              <td class="px-4 py-2">{{ bond.user.name }}</td>
              <td class="px-4 py-2 cursor-pointer">
                <button
                  class="btn-block btn"
                  style="width:140px; background-color:#27ae60; color:white; border-radius:17px"
                  @click="downloadBondFile(bond.insurance_bond_file)"
                >
                  Download PDF
                </button>
              </td>
              <td class="px-4 py-2">{{ formatDate(bond.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
  
      <div v-if="isSearched && insuranceBonds.length > 0" class="flex justify-center mt-4">
        <button
  ...
  async function downloadBondFile(fileUrl) {
    try {
  ...
  function exportToExcel() {
    ...
    const worksheet = XLSX.utils.json_to_sheet(
      paginatedData.value.map((entry, index) => ({
        ...
        InsuranceBondFile: entry.insurance_bond_file || 'N/A',
  ...
  function exportToPDF() {
    ...
    const doc = new jsPDF();
    const title = 'Insurance Bond Reports Data';
    const headers = [['No', 'Tender Title', 'Created By', 'Created At']]; // Excluded Insurance Bond File
