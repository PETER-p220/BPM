    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white dark:bg-neutral-800 rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <div v-if="selectedProject" class="space-y-4">
          ...
          <div>
            <strong>Created By:</strong> {{ selectedProject.created_by || 'NA' }}
          </div>
          <div>
          ...
          <div>
            <strong>Contract PDF:</strong> {{ selectedProject.contract?.pdf_file || 'NA' }}
          </div>
          <div>
          ...
          <div>
            <strong>Tender Attachment:</strong> {{ selectedProject.tender?.attachment || 'NA' }}
          </div>
        </div>
      </div>
    </div>
...
function exportToExcel() {
  const exportData = filteredProjects.value.map(project => ({
    ...
    'Contract Timeline Category': project.contract.time_line_category,
    'Contract Start Date': formatDate(project.contract.start_date),
    'Contract End Date': formatDate(project.contract.end_date),
    'Contract PDF': project.contract.pdf_file,
    'Contract Status': project.contract.status,
    'Performance Guarantee': project.contract.performance_guarantee,
    'Tender Type': project.tender.tender_type,
    'Tender Attachment': project.tender.attachment,
...
function exportToPDF() {
  ...
  autoTable(doc, {
    head: ['No', 'Project Name', 'Engineer', 'Team Members', 'Created By', 'Start Date', 'End Date', 'Contract Title', 'Created At', 'Status', 'Extension Date', 'Follow Up'],
    body: tableData,
  });
