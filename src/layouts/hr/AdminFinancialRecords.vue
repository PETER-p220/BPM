  ...
  <div class="financial-records-container">
    ...
    <div class="dashboard-header">
      <div class="header-content">
        <div class="header-left">
          ...
          <p class="dashboard-subtitle">Administrative financial overview and management</p>
        </div>
    ...
    <div class="analytics-section">
      <div class="analytics-card">
        ...
        <div class="activity-list">
          <div v-for="user in stats.userActivity" :key="user.created_by" class="activity-item">
            <div class="user-info">
              <strong>{{ user.creator?.name || 'Unknown' }}</strong>
              <span>{{ user.records_created }} records</span>
            </div>
            <div class="amount-info">
              TZS {{ formatNumber(user.total_amount) }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="analytics-card">
        ...
        <div class="category-list">
          <div v-for="category in stats.categoryBreakdown" :key="category.category" class="category-item">
            <div class="category-info">
              <strong>{{ category.category }}</strong>
              <span>{{ category.count }} records</span>
            </div>
            <div class="category-amounts">
              <div class="income">Income: TZS {{ formatNumber(category.income) }}</div>
              <div class="expense">Expenses: TZS {{ formatNumber(category.expenses) }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="charts-section">
      ...
      <div class="chart-container">
        ...
        <div class="chart-wrapper">
          <canvas ref="userPerformanceChart" width="400" height="200"></canvas>
        </div>
      </div>
    </div>
            ...
            <tr v-for="record in records" :key="record.id">
              ...
              <td>
                <div class="action-buttons">
                  <button @click="viewRecord(record)" class="action-btn view-btn">
                    <i class="fas fa-eye"></i>
                  </button>
                  <button v-if="record.status === 'pending'" @click="approveRecord(record)" class="action-btn approve-btn">
...
const userPerformanceChart = ref(null)

// Chart instances
let monthlyTrendsInstance = null
let categoryInstance = null
let comparisonInstance = null
let userPerformanceInstance = null
...
const fetchRecords = async () => {
  try {
    const response = await axios.get('/api/admin/financial/records', {
...
const fetchStats = async () => {
  try {
    const response = await axios.get('/api/admin/financial/records/stats')
...
const exportPDF = async () => {
  try {
    const response = await axios.get('/api/admin/financial/records/export/pdf', {
      params: filters.value,
      responseType: 'blob'
    })
...
const exportExcel = async () => {
  try {
    const response = await axios.get('/api/admin/financial/records/export/excel', {
      params: filters.value,
      responseType: 'blob'
    })
...
const viewRecord = (record) => {
  ...
  toast.info(`Viewing record #${record.id}`)
}

const approveRecord = async (record) => {
  try {
    const response = await axios.put(`/api/financial/records/${record.id}`, {
      ...record,
      status: 'approved'
    })
...
const initializeCharts = () => {
  ...
  if (userPerformanceChart.value) {
    const ctx = userPerformanceChart.value.getContext('2d')
    userPerformanceInstance = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: [],
        datasets: [
...
const generateUserLabels = () => {
  return ['John Doe', 'Jane Smith', 'Mike Johnson', 'Sarah Wilson', 'Tom Brown']
}

const generateUserData = () => {
  return [12, 19, 8, 15, 11]
}
...
const updateCharts = () => {
  ...
  // Update User Performance Chart with real data
  if (stats.value.userActivity && userPerformanceInstance) {
    const userActivityData = stats.value.userActivity
    const labels = userActivityData.map(item => item.creator?.name || 'Unknown User')
    const data = userActivityData.map(item => parseInt(item.records_created) || 0)
    
    userPerformanceInstance.data.labels = labels
    userPerformanceInstance.data.datasets[0].data = data
    userPerformanceInstance.update()
  }
}
...
onMounted(async () => {
  ...
  if (stats.value.monthlyTrends || stats.value.categoryBreakdown || stats.value.userActivity) {
    await nextTick()
    updateCharts()
  }
})
