<template>
    <div class="p-4 space-y-4" style="font-family: 'Trirong', sans-serif; font-size: 17px">
      <PageHeader subtitle="Awarded Tenders">
        <div class="flex flex-col sm:flex-row sm:space-x-2">
          <router-link to="/admin-create-awardedtender">
            <BaseButton @click="CreateAwardedTender" style="background-color: #2e4053;" class="w-full sm:w-auto">
        ...
        <table class="w-full divide-y divide-gray-200 rounded-table dark:divide-gray-700" id="data-table"
          ...
          <tbody class="bg-white divide-y divide-gray-200 dark:bg-dark-header dark:divide-gray-700">
            <tr v-for="(award, index) in filteredData" :key="award.award_id">
              ...
              <td class="table-data">
                <icon  @click="acceptAward(award.award_id)" class="fas fa-edit"  style="color:#196f3d; font-weight:bold; font-size:17px">
                </icon>
              </td>
            </tr>
          </tbody>
  ...
  async function fetchAwardedTenders() {
    try {
      const response = await axios.get('api/awarded-tender'); // API to fetch awarded tenders
  ...
  // Navigate to edit awarded tender page
  function acceptAward(awardId) {
    router.push({ name: 'AdminEditAwardedTender', params: { award_id: awardId } });
  }
