      ...
      <table class="w-full divide-y divide-gray-200 rounded-table dark:divide-gray-700">
        ...
        <tbody class="bg-white divide-y divide-gray-200 dark:bg-dark-header dark:divide-gray-700">
          <tr v-for="(assignedTender, index) in paginatedData" :key="assignedTender.assign_id">
            ...
            <td class="table-data">{{ assignedTender.user_name }}</td>
            <td class="table-data">
              <button 
...
// Computed property to paginate the filtered data
...
function getStatusText(tender) {
  ...
  return formatDate(tender.expired_at, true);
}
...
// Navigate to the edit tender page
function editAssignedTender(assignId) {
  router.push({ name: 'EditAssignedTender', params: { assign_id: assignId } });
}

// Dummy function for addNewUser
function addNewUser() {
  // Placeholder for Assign Tender button click logic if needed
}
</script>