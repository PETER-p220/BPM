<template>
    <div class="py-5 add-user" style="font-family: 'cygre', serif; font-size: 23px">
      <div class="container px-4 mx-auto">
        <div class="w-full shadow-lg card">
          <div class="flex items-center justify-between px-4 py-2 text-white card-header" style="background-color: #283747;">
            <div><i class="mr-2 fa fa-plus"></i> Assign Tender</div>
            <button type="button" class="text-white" @click="closeModal">
              <i class="fa fa-times"></i>
            </button>
          </div>
  
          <!-- Tender Assignment Form -->
          <div class="p-6 bg-white card-body">
            <h3 class="mb-4 text-lg font-semibold">Assign Tender</h3>
            
            <!-- First Row: Tender and Engineer -->
            <div class="row">
              <div class="col-sm-6">
                <label for="tenderSelect" class=" form-label">Select Tender</label>
                <select id="tenderSelect" class="w-full px-3 py-2 border rounded form-control" v-model="assignmentData.tender_id">
                  <option v-for="tender in tenders" :key="tender.tender_id" :value="tender.tender_id">{{ tender.title }}</option>
                </select>
              </div>
              <div class="col-sm-6">
                <label for="engineerSelect" class=" form-label">Select Engineer</label>
                <select id="engineerSelect" class="w-full px-3 py-2 border rounded form-control" v-model="assignmentData.user_id">
                  <option v-for="user in engineers" :key="user.user_id" :value="user.user_id">{{ user.name }}</option>
                </select>
              </div>
            </div>
  
            <!-- Second Row: Head of Department (HOD) and Status -->
            <div class="mt-4 row">
              <div class="col-sm-6">
                <label for="hodSelect" class=" form-label">Select Head of Department (HOD)</label>
                <select id="hodSelect" class="w-full px-3 py-2 border rounded form-control" v-model="assignmentData.is_sent_to_hod">
                  <option v-for="user in hods" :key="user.user_id" :value="user.user_id">{{ user.name }}</option>
                </select>
              </div>
              <div class="col-sm-6">
                <label for="statusSelect" class=" form-label">Select Status</label>
                <select id="statusSelect" class="w-full px-3 py-2 border rounded form-control" v-model="assignmentData.status">
                  <option value="assigned">Assigned</option>
                  <option value="cancelled">Cancelled</option>
                </select>
              </div>
            </div>
  
            <!-- Third Row: Date of Submission and Expiry Date -->
            <div class="mt-4 row">
              <div class="col-sm-6">
                <label for="dateOfSubmission" class=" form-label">Date of Submit Tender</label>
                <input type="date" id="dateOfSubmission" class="w-full px-3 py-2 border rounded form-control" v-model="assignmentData.date_of_submission" />
              </div>
              <div class="col-sm-6">
                <label for="expireDate" class=" form-label">Expire Date</label>
                <input type="date" id="expireDate" class="w-full px-3 py-2 border rounded form-control" v-model="assignmentData.expire_date" />
              </div>
            </div>
  
            <!-- Action Buttons -->
            <div class="flex gap-4 mt-4">
              <button @click="assignTender" class="px-4 py-2 mt-4 text-white rounded hover:bg-blue-700" style="background-color:#283747;">
                <i class="fa fa-plus"></i> Assign Tender
                <span v-if="isLoading" class="flex items-center gap-2 mt-4">
                  <i class="fa fa-spinner fa-spin"></i> Loading...
                </span>
              </button>
              <span>
                <router-link to="/adminview-assignedtenders" class="flex items-center gap-2 px-4 py-2 text-gray-700 bg-gray-200 rounded hover:bg-gray-300">
  <i class="fa fa-times"></i> Cancel
</router-link>

              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </template>
  
  
  <script setup>
  import { ref, onMounted } from 'vue';
  import axios from '@/axios';
  import { useToast } from 'vue-toastification';
  import { useRouter } from 'vue-router';
  
  const toast = useToast();
  const router = useRouter();
  const assignmentData = ref({
    tender_id: '',
    user_id: '',
    status: 'assigned',
    date_of_submission: '',
    expire_date: '',
    is_sent_to_hod: '',
  });
  const tenders = ref([]);
  const engineers = ref([]);
  const hods = ref([]); // New array for HODs
  const isLoading = ref(false);
  
  // Fetch tenders and engineers on component mount
  onMounted(async () => {
    await fetchTenders();
    await fetchEngineers();
    await fetchHods(); // Fetch HODs
  });
  
  // Fetch tenders from API
  async function fetchTenders() {
    try {
      const response = await axios.get('api/dropdown/tender');
      tenders.value = response.data.data;
    } catch (error) {
      handleError(error);
    }
  }
  
  // Fetch engineers from API
  async function fetchEngineers() {
    try {
      const response = await axios.get('api/dropdown/engineer');
      engineers.value = response.data.users;
    } catch (error) {
      handleError(error);
    }
  }
  
  // Fetch HODs from API
  async function fetchHods() {
    try {
      const response = await axios.get('api/dropdown/hod'); // Ensure this API exists
      hods.value = response.data.users; // Assuming the response structure
    } catch (error) {
      handleError(error);
    }
  }
  
  // Assign tender to user
  async function assignTender() {
    isLoading.value = true;
  
    // Validation
    if (!assignmentData.value.tender_id || !assignmentData.value.user_id || !assignmentData.value.date_of_submission || !assignmentData.value.expire_date) {
      toast.error('Please fill in all required fields');
      isLoading.value = false;
      return;
    }
  
    // If "Sent to HOD" is checked, use the HOD's user_id and send it under is_sent_to_hod
    if (assignmentData.value.is_sent_to_hod && assignmentData.value.hod_id) {
      assignmentData.value.is_sent_to_hod = assignmentData.value.hod_id; // Assign HOD's user_id to is_sent_to_hod
    }
  
    try {
      const response = await axios.post('api/assign/tender', assignmentData.value);
      toast.success(response.data.message);
      assignmentData.value = {
        tender_id: '',
        user_id: '',
        status: 'assigned',
        date_of_submission: '',
        expire_date: '',
        is_sent_to_hod: '',
      };
      router.push('/adminview-assignedtenders');
    } catch (error) {
      handleError(error);
    } finally {
      isLoading.value = false;
    }
  }
  
  // Handle errors
  function handleError(error) {
    let message = 'An unexpected error occurred';
    if (error.response && error.response.data && error.response.data.message) {
      message = error.response.data.message;
    } else if (error.request) {
      message = 'No response from the server. Please check your connection.';
    } else {
      message = error.message;
    }
    toast.error(message);
  }
  </script>
  