<template>
    <div class="p-4 space-y-4"  style="font-family: 'cygre', serif; font-size: 17px;box-shadow: rgba(0, 0, 0, 0.16) 0px 10px 36px 0px, rgba(0, 0, 0, 0.06) 0px 0px 0px 1px;">
      ...
      <div class="py-2 space-y-3" >
        ...
        style="box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 3px 0px, rgba(0, 0, 0, 0.06) 0px 1px 2px 0px;">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            style="box-shadow: rgba(0, 0, 0, 0.1) 0px 10px 50px;">
              <div class="flex flex-col">
                <img v-if="project.activity_photo" :src="project.activity_photo" class="object-cover w-full h-48 mb-4" alt="Activity Image" />
                <div class="mb-2 text-lg font-semibold text-red-700"><strong class="font-semibold">User:</strong>{{ project.creator_name }}</div>
                ...
                <button class="text-xs text-gray-700"  style="width:180px;background-color:#196f3d;color:white;border-radius:17px">  <strong class="font-semibold text-white">Task Status:</strong> {{ project.task_status }}</button>
                <br>
  ...
  async function fetchProjects() {
    try {
      const response = await axios.get('api/hod/project/activities');
  ...
  function viewProjectActivities(activity_id) {
    router.push({ name: 'ApproveActivity', params: { activity_id } });
  }
