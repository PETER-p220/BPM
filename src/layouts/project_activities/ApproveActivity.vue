<template>
  ...
  >
    <div
      class="p-6 bg-white rounded-lg shadow-lg w-[500px] max-h-[90vh] overflow-y-auto"
    >
      ...
      <form @submit.prevent="saveChanges">
        ...
        <div class="mt-4">
          <label for="is_viewed" class="font-semibold form-label">Approve Activity</label>
          <select
            id="is_viewed"
            v-model="form.is_viewed"
            class="w-full px-3 py-2 border rounded form-control"
          >
            <option value="">None</option>
            <option value="viewed">Approved</option>
            <option value="pending">Pending</option>
          </select>
        </div>
...
async function saveChanges() {
  ...
  try {
    ...
    if (response.data.status === 'success') {
      ...
      router.push('/hodview-activity'); // Navigate to the specified route after success
    } else {
