<template>
    <div class="p-4 space-y-4"  style="font-family: 'cygre', serif; font-size: 17px">
      <PageHeader subtitle="Projects activities">
        <div class="flex flex-col sm:flex-row sm:space-x-2">
          <router-link to="/submit-activity">
            <BaseButton @click="addNewAnalysis" style="background-color: #2e4053;" class="w-full sm:w-auto">
              Submit Activity here
  ...
  async function fetchProjects() {
    try {
      const response = await axios.get('api/eng/project/activities');
  ...
  function viewProjectActivities(activity_id) {
    router.push({ name: 'ActivityDetailsPage', params: { activity_id } });
  }
