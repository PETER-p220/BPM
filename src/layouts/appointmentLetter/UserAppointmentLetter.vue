<template>
    <div class="p-4 space-y-4" style="font-family: 'cygre', sans-serif; font-size: 17px">
      <PageHeader subtitle="Appointment Letters">
      ...
      <div class="overflow-x-auto">
        <table class="w-full divide-y divide-gray-200 rounded-table dark:divide-gray-700" id="data-table"
          ...
          <tbody class="bg-white divide-y divide-gray-200 dark:bg-dark-header dark:divide-gray-700">
            <tr v-for="(appointmentLetter, index) in paginatedData" :key="appointmentLetter.letter_id">
              <td class="table-data">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
              <td class="table-data">{{ appointmentLetter.tender.title }}</td>
              <td class="table-data">
              ...
              <td class="table-data">{{ formatDate(appointmentLetter.created_at) }}</td>
              <td class="table-data">
                <router-link v-if="appointmentLetter.status === 'pending'"
                  :to="{ name: 'AcceptAppointmentLetter', params: { letter_id: appointmentLetter.letter_id }}"
                  class="btn-block btn inline-block text-center"
                  style="width:140px;background-color:#1e90ff;color:white;border-radius:17px">
                  Accept
                </router-link>
              </td>
            </tr>
  ...
  function exportToExcel() {
    const worksheet = XLSX.utils.json_to_sheet(
      paginatedData.value.map((entry, index) => ({
        ...
        LetterFile: entry.letter_file || 'N/A',
    ...
    XLSX.writeFile(workbook, 'Appointment_Letters.xlsx');
  }
  
  // Export to PDF
  function exportToPDF() {
    const doc = new jsPDF();
    const title = 'Appointment Letters Data';
    const headers = [['No', 'Tender Title', 'Letter File', 'Status', 'Created At']];
    const data = paginatedData.value.map((entry, index) => [
      (currentPage.value - 1) * itemsPerPage + index + 1,
      entry.tender?.title || 'N/A',
      entry.letter_file || 'N/A',
      entry.status || 'N/A',
    ...
    doc.save('Appointment_Letters.pdf');
  }
  </script>